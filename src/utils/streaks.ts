...
/** Read state safely. If feature is OFF, don't touch storage; return zeros. */
export function loadStreakState(): StreakState {
  if (!FEATURE_STREAKS) {
    return { lastPlayed: "", streak: 0, freeze: 0, perfectSinceFreeze: 0 };
  }
...
/** Persist state. If feature is OFF, no-op (no writes). */
export function saveStreakState(s: StreakState) {
  if (!FEATURE_STREAKS) return;
...
/** Update streaks at the end of a run. If feature is OFF, do nothing. */
export function updateStreakOnFinish(score: number, total: number) {
  if (!FEATURE_STREAKS) return; // hard gate
...
/** Legacy helpers for hiding the pill (kept as-is). */
